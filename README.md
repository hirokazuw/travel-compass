...
ホテルの通常検索を国内・海外ともApifyのGoogle Hotels Actorへ統一しました。国内・海外タブは予約サイト導線の切替にだけ使用し、楽天トラベルAPI、Scrape.do、SerpAPIの既存実装は将来の比較・フォールバック用として維持しています。
...
* 航空券は既存取得元へのフォールバック処理を維持
* 楽天トラベルAPI、Scrape.do、SerpAPI、AeroDataBox、Travelpayouts関連コードは互換・比較用として維持
