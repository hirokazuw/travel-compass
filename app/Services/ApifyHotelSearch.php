...
final class ApifyHotelSearch
{
    ...
    public function search(string $destination, string $checkIn, string $checkOut, int $adults, int $children): array
    {
        ...
        if ($children > 0)
            $input['children_ages'] = implode(',', array_fill(0, $children, '8'));
        return $this->normalizer->normalizeHotels(
            $this->cache->remember($input, fn(): array => $this->client->run('hotels_url', $input))
        );
    }
}
