...
final class FerrySearchRequest
{
    ...
    public static function fromPost(array $input, string $sessionToken): self
    {
        ...
        } elseif (RequestValidation::integer($values['ferry_company_id'], 1) === false) {
            $errors[] = 'フェリー会社の選択内容が無効です。';
        }
        if ($values['ferry_route_id'] === '') {
            $errors[] = '航路を選択してください。';
        } elseif (RequestValidation::integer($values['ferry_route_id'], 1) === false) {
            $errors[] = '航路の選択内容が無効です。';
        }
        if (!RequestValidation::lengthBetween($values['ferry_company_name'], 0, 150)) $errors[] = 'フェリー会社名が長すぎます。';
