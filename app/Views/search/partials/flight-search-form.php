<?php if ($errors && $activeFlightScope === $flightScope): ?>
...
<form method="post" class="flight-search-form">
    ...
    <input type="hidden" name="flight_scope" value="<?= $h($flightScope) ?>">
    ...
    <label>出発地<input name="origin" value="<?= $h($values['origin']) ?>" placeholder="例：<?= $flightScope === 'domestic' ? '東京' : '東京（TYO）' ?>" required></label>
    <label>目的地<input name="destination" value="<?= $h($values['destination']) ?>" placeholder="例：<?= $flightScope === 'domestic' ? '札幌' : 'ソウル（SEL）' ?>" required></label>
